...
namespace symdump.exefile.operands
{
    ...
    {
        public delegate bool ArgPredicate<in T2>(T instruction, T2 operand) where T2 : IOperand?;

        public delegate bool InsnPredicate(T instruction);

        private readonly T? _instruction;
        private readonly Matcher _matcher;
        private int _argIndex;


        internal InsnMatcher(Matcher matcher, T? instruction)
        {
            _matcher = matcher;
            _instruction = instruction;
        }

        public InsnMatcher<T> Arg<T2>(out T2? operand, ArgPredicate<T2>? argPredicate = null)
        ...
        public InsnMatcher<T> Arg<T2>(ArgPredicate<T2>? argPredicate = null) where T2 : class, IOperand
        {
        ...
        public InsnMatcher<T> Where(InsnPredicate insnPredicate)
        {
            if (!_matcher.Matches)
                return this;
