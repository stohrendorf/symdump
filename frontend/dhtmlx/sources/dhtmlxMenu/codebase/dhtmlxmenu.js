...
function dhtmlXMenuObject(base, skin) {
    ...
    this._addSubItemToSelected = function (item, polygon) {
        ...
        for (var q = 0; q < this.conf.selected_sub.length; q++) {
            if ((this.conf.selected_sub[q][0] == item) && (this.conf.selected_sub[q][1] == polygon)) {
                t = false;
            }
        }
        if (t == true) {
            this.conf.selected_sub.push([item, polygon]);
        }
        return t;
    };
    this._removeSubItemFromSelected = function (item, polygon) {
        ...
        for (var q = 0; q < this.conf.selected_sub.length; q++) {
            if ((this.conf.selected_sub[q][0] == item) && (this.conf.selected_sub[q][1] == polygon)) {
                ...
                m[m.length] = this.conf.selected_sub[q];
            }
        }
        if (t == true) {
            this.conf.selected_sub = m;
        }
        return t;
    };
    this._getSubItemToDeselectByPolygon = function (polygon) {
        ...
        for (var q = 0; q < this.conf.selected_sub.length; q++) {
            if (this.conf.selected_sub[q][1] == polygon) {
                ...
                var t = true;
                for (var w = 0; w < this.conf.opened_poly.length; w++) {
                    if (this.conf.opened_poly[w] == this.conf.selected_sub[q][0]) {
                        t = false;
                    }
                }
                if (t == true) {
                    this.conf.opened_poly[this.conf.opened_poly.length] = this.conf.selected_sub[q][0];
                }
    ...
    this._redistribSubLevelSelection = function (id, parentId) {
        ...
        for (var q = 0; q < this.conf.opened_poly.length; q++) {
            if (this.conf.opened_poly[q] != parentId) {
                this._hidePolygon(this.conf.opened_poly[q]);
            }
        }
    ...
    this._clearAndHide = function () {
        ...
        while (that.conf.opened_poly.length > 0) {
            that.conf.opened_poly.pop();
        }
...
dhtmlXMenuObject.prototype._redistribTopLevelSelection = function (id, parent) {
    ...
    if (this.itemPull[this.idPrefix + id]["state"] == "enabled") {
        ...
        if ((this.itemPull[this.idPrefix + id]["complex"]) && (this.conf.selected != -1)) {
            this._showPolygon(this.idPrefix + id, this.conf.dir_toplv);
        }
    }
};
...
dhtmlXMenuObject.prototype._renderToplevelItem = function (id, pos) {
                ...
                if ((that.itemPull[this.id]["complex"]) && (that.conf.selected != -1)) {
                    if (that.conf.top_mode && that.conf.mode == "web" && !that.conf.context) {
                        ...
                        this._menuOpenTM = window.setTimeout(function () {
                            that._showPolygon(showItemId, that.conf.dir_toplv);
                        }, that.conf.top_tmtime);
                    } else {
    ...
    m.onmouseout = function () {
        ...
        if (that.conf.mode == "web") {
            window.clearTimeout(that.conf.tm_handler);
            that.conf.tm_handler = window.setTimeout(function () {
                that._clearAndHide();
            }, that.conf.tm_sec, "JavaScript");
        }
        if (that.conf.top_mode && that.conf.mode == "web" && !that.conf.context) {
    ...
    m.onclick = function (e) {
        ...
        // fix, added in 0.4
        if (that.conf.mode != "web" && that.itemPull[this.id]["state"] == "disabled") {
            return;
        }
        //
        e = e || event;
        ...
        if (that.conf.mode == "win") {
            if (that.itemPull[this.id]["complex"]) {
                if (that.conf.selected == this.id) {
                    that.conf.selected = -1;
                    var s = false;
                } else {
                    that.conf.selected = this.id;
                    var s = true;
                }
                if (s) {
                    that._showPolygon(this.id, that.conf.dir_toplv);
                } else {
                    that._hidePolygon(this.id);
                }
            }
        }
...
dhtmlXMenuObject.prototype._renderSublevelItem = function (id, pos) {
    ...
    tr.onmouseout = function () {
        if (that.conf.mode == "web") {
            ...
            that.conf.tm_handler = window.setTimeout(function () {
                if (that && that._clearAndHide) that._clearAndHide();
            }, that.conf.tm_sec, "JavaScript");
        }
        var k = this;
        if (that.conf.hide_tm[this.id]) window.clearTimeout(that.conf.hide_tm[this.id]);
        that.conf.hide_tm[this.id] = window.setTimeout(function () {
            k._visible = false;
        }, 50);
    };
...
dhtmlXMenuObject.prototype.addContextZone = function (zoneId) {
    ...
    for (var a in this.conf.ctx_zones) {
        zoneExists = zoneExists || (a == zoneId) || (this.conf.ctx_zones[a] == zone);
    }
    ...
    if (window.dhx4.isOpera) {
        this.operaContext = function (e) {
            that._doOnContextMenuOpera(e, that);
        };
...
dhtmlXMenuObject.prototype.isContextZone = function (zoneId) {
    ...
    var isZone = false;
    if (this.conf.ctx_zones[zoneId] != null) {
        if (this.conf.ctx_zones[zoneId] == document.getElementById(zoneId)) isZone = true;
    }
    return isZone;
};
...
dhtmlXMenuObject.prototype._showContextMenu = function (x, y, zoneId) {
    ...
    this.idPull[this.idPrefix + this.topId] = [x, y];
